...
`src/systematic/`은 이 문제를 해결하는 주 실험 경로다. 기존 ball-swap 경로는
회귀 테스트와 보조 과제로 보존한다.
...
Adaptive halting은 고정-loop H1–H3의 주 조건에 넣지 않는다. overthinking과
계산량 절감을 확인하는 후속 ablation으로만 사용해야 한다. 주 실험은 먼저 loop
sweep 전체를 저장해 accuracy뿐 아니라 hidden norm과 마지막 update ratio를 함께
분석한다.
