# Ball-Swap 모델링 및 안정화 ablation 매뉴얼

> 이 문서는 현재 주 실행 경로인 `src/original/` ball-swap 모델을 설명한다.
> `src/systematic/` object-location은 이후 confound를 분리하기 위한 controlled
> 확장 실험으로 사용한다.
...
- **loop-count conditioning**: 각 반복에 learned loop embedding을 입력
...
유지하고 adaptive halting은 `--adaptive-kl-eval` 조건으로 별도 보고한다.
...
최소 3개 seed(`0, 1, 2`)로 반복한다. 결과와 checkpoint는 `runs/original/`에
저장된다.
...
```bash
python scripts/run_original_experiments.py \
  --architecture recurrent-r0 --smoke --device cpu \
...
```bash
python scripts/run_original_experiments.py \
  --architecture recurrent-r0 --num-loops 6 \
  --loop-conditioning learned --residual-scale 0.5 \
...
`runs/original/*.json`의 `by_swaps`와 `loop_sweep`을 사용하면 교환 횟수와 inference
loop 수에 따른 붕괴 곡선을 바로 그릴 수 있다.
